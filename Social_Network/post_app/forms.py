...
class PostForm(forms.ModelForm):
    ...
    images = MultipleFileField(
        label= '',
        required = False,
        widget= MultipleFileInput(attrs={'multiple': True, 'accept': 'image/*', 'class': 'image-post'}),
    )
    ...
    class Meta:
        ...
        widgets = {
            ...
            'content': forms.Textarea(attrs={'rows': 5, 'placeholder': 'Текст посту', 'id': 'id_content'}),
            # 'links': forms.TextInput(attrs={'placeholder': 'Посилання на публікацію'})
        }

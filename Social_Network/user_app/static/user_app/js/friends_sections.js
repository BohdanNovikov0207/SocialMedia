...
const homeBlock = document.querySelector("#main-friends");
...
const sectionList = document.querySelector("#in-requests_friends");
...
const backHomeButton = document.querySelector("#main-friends");
...
export async function loadSectionPage(section, page) {
  // if (isLoading) return; // Якщо вже йде завантаження, не запускаємо нове.
  ...
  document.querySelector(".all_friends").lastElementChild.appendChild(sectionBlock)
  sectionList.appendChild(friendsSentinel);
...
export async function openSection(section) {
  ...
  if (friendsSentinel){
    sectionBlock.appendChild(friendsSentinel);
  }
...
export const friendsObserver = new IntersectionObserver(async (entries) => {
...
export const sectionButtons = document.querySelectorAll("[data-section-link]");
